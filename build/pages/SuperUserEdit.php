...
$sql = "SELECT * FROM pengguna WHERE nim='$id_nim'";
...
if (mysqli_num_rows($result) > 0) {
  while($row = mysqli_fetch_assoc($result)) {
    ...
    <main class="relative h-full max-h-screen transition-all duration-200 ease-in-out xl:ml-68 rounded-xl">
        ...
        <nav class="relative flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all ease-in shadow-none duration-250 rounded-2xl lg:flex-nowrap lg:justify-start"
            ...
            <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
                <nav>
                    ...
                    <h6 class="mb-0 font-bold text-black capitalize">Data User</h6>
                </nav>
        ...
        <div class="bg-blue-50 w-full h-full p-5 rounded overflow-auto">
            
            <!-- Modal Body -->
            <!-- Edit akun -->
            <form action="../proses/prosesedituser.php" method="POST">
                <div class="w-full bg-white p-4 h-max overflow-y-auto rounded-3 shadow-dark-blur">
                    <h1 class="text-2xl font-bold mb-4">Edit Akun User</h1>
                    <div class="px-0 md:px-7 lg:px-16">
                        <div class="flex flex-wrap w-full justify-between px-2 xl:px-7 mb-5">
                            <p class="font-semibold mr-4 mb-2 xl:text-xl">NIM</p>
                            <input type="text" name="nim" value="<?= $row["nim"] ?>" class="flex border h-8 w-full md:w-3/4 pl-2 rounded-1">
                        </div>
                        <div class="flex flex-wrap w-full justify-between px-2 xl:px-7 mb-5">
                            <p class="font-semibold mr-4 mb-2 xl:text-xl">Username</p>
                            <input type="text" name="username" value="<?= $row["username"] ?>" class="flex border h-8 w-full md:w-3/4 pl-2 rounded-1">
                        </div>
                        <div class="flex flex-wrap w-full justify-between px-2 xl:px-7 mb-5">
                            <p class="font-semibold mr-4 mb-2 xl:text-xl">Email</p>
                            <input type="text" name="email" value="<?= $row["email"] ?>" class="flex border h-8 w-full md:w-3/4 pl-2 rounded-1">
                        </div>
                        
                        <div class="flex flex-wrap w-full justify-between px-2 xl:px-7 mb-5">
                            <p class="font-semibold mr-4 mb-2 xl:text-xl">Level</p>
                            <span class="flex border h-8 w-full md:w-3/4 pl-2 rounded-1">user</span>
                            <input type="hidden" name="level" value="user">
                        </div>
                        <div class="flex justify-center md:justify-end px-2 xl:px-7">
                            <button type="submit" class="bg-[#263959] px-5 text-white rounded-lg h-9 font-semibold hover:bg-[#EBA92E]">
