...
<body class="m-0 font-sans text-base antialiased font-normal bg leading-default bg-blue-50 text-slate-500">
    ...
    <main class="max-w-screen-xl mx-auto p-7 py-10">
        <h1 class="text-2xl font-bold my-5">Pertanyaan</h1>
        <form action="../proses/proseseditfaq.php?id=<?= $id ?>" method="POST">
        <textarea name="pertanyaan" class="w-full bg-white p-4 h-44 overflow-y-auto rounded-3 shadow-dark-blur border-2 pl-2 pt-1"
         rows="4" cols="50"><?= htmlspecialchars($row["pertanyaan"]) ?></textarea>
            <h1 class="text-2xl font-bold my-5">Jawaban</h1>
        <textarea name="jawaban" class="w-full bg-white p-4 h-44 overflow-y-auto rounded-3 shadow-dark-blur border-2 pl-2 pt-1" 
        rows="4" cols="50"><?= htmlspecialchars($row["jawaban"]) ?></textarea>
