...
<body class="m-0 font-sans text-base antialiased font-normal leading-default bg-blue-50 text-slate-500">
  ...
  <main class="relative h-full max-h-screen transition-all duration-200 ease-in-out xl:ml-68 rounded-xl">
    ...
    <?php
          ...
          $sql = "SELECT layanan, COUNT(*) AS jumlah FROM antrian GROUP BY layanan";
          $result = mysqli_query($conn, $sql);
          
          // Memasukkan hasil query ke dalam array $jumlah_layanan
          $jumlah_layanan = [];
          while ($row = mysqli_fetch_assoc($result)) {
              $jumlah_layanan[$row['layanan']] = $row['jumlah'];
          }
    ...
    <div class="w-full px-10 py-6 mx-auto">
      <div class="flex flex-wrap -mx-3">
        ...
        <div class="w-full max-w-full xl:h-32 px-3 mb-6 sm:w-1/2 sm:flex-none ">
          <div class="flex bg-white h-full justify-between rounded-2xl shadow-dark-blur">
            <div class="p-4 mr-3 w-full ">
              <h5 class="m-0">Umum</h5>
              <p class="mb-0 ">Peminjaman sarana dan prasaranan, permintaan barang.</p>
            </div>
            <div class="bg-[#E9B830] w-1/4 flex items-center rounded-r-2xl justify-center">
              <h1 class="mb-0 text-white"><?= $jumlah_layanan[4] ?? 0 ?></h1>
            </div>
          </div>
        </div>
        <div class="w-full max-w-full xl:h-32 px-3 mb-6 sm:w-1/2 sm:flex-none ">
          <div class="flex bg-white h-full justify-between rounded-2xl shadow-dark-blur">
            <div class="p-4 mr-3 w-full">
              <h5 class="m-0">Akademik</h5>
              <p class="mb-0">Daftar ulang mahasiswa baru, daftar ulang mahasiswa lama, dll</p>
            </div>
            <div class="bg-[#E9B830] w-1/4 flex items-center rounded-r-2xl justify-center">
              <h1 class="mb-0 text-white"><?= $jumlah_layanan[1] ?? 0 ?></h1>
            </div>
          </div>
        </div>
        <div class="w-full max-w-full xl:h-32 px-3 mb-6 sm:w-1/2 sm:flex-none ">
          <div class="flex bg-white h-full justify-between rounded-2xl shadow-dark-blur">
            <div class="p-4 mr-3 w-full">
              ...
              <p class="mb-0">Pengajuan Dana Kegiatan Bidang Kemahasiswaan Fakultas Vokasi,
                dll</p>
            </div>
            <div class="bg-[#E9B830] w-1/4 flex items-center rounded-r-2xl justify-center">
              <h1 class="mb-0 text-white"><?= $jumlah_layanan[3] ?? 0 ?></h>
            </div>
          </div>
        </div>
        <div class="w-full max-w-full xl:h-32 px-3 mb-6 sm:w-1/2 sm:flex-none ">
          <div class="flex bg-white h-full justify-between rounded-2xl shadow-dark-blur">
            <div class="p-4 mr-3 w-full">
              <h5 class="m-0">Kemahasiswaan</h5>
              <p class="mb-0">Pengajuan Beasiswa, Pengajuan Reward Prestasi</p>
            </div>
            <div class="bg-[#E9B830] w-1/4 flex items-center rounded-r-2xl justify-center">
              <h1 class="mb-0 text-white"><?= $jumlah_layanan[2] ?? 0 ?></h1>
            </div>
          </div>
        </div>
